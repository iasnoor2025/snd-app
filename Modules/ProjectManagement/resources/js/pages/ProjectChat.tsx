                ...
                <div className="space-y-2">
                    {messages.map((msg) => (
                        <div key={msg.id} className={`flex ${msg.user_id === currentUser.id ? 'justify-end' : 'justify-start'}`}>
                            <div
                                className={`rounded-lg px-3 py-2 ${msg.user_id === currentUser.id ? 'bg-blue-100 text-blue-900' : 'bg-gray-100 text-gray-900'}`}
                            >
                                <div className="text-xs font-semibold">{msg.user?.name || 'User'}</div>
                                <div>{msg.message}</div>
                                <div className="text-right text-[10px] text-gray-500">{new Date(msg.created_at)}</div>
                            </div>
                        </div>
                    ))}
