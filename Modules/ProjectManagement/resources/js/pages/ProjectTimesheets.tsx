          ...
          timesheets.map((ts) => (
            <div key={ts.id} className="border rounded p-4 flex flex-col md:flex-row md:items-center md:justify-between">
              ...
              <div className="flex gap-2 mt-2 md:mt-0">
                <Button variant="outline" onClick={() => handleOpen(ts)}>Edit</Button>
                <Button variant="destructive" onClick={() => handleDelete(ts.id)}>Delete</Button>
                {ts.status !== 'approved' && (
                  <Button variant="success" onClick={() => handleApprove(ts.id)}>Approve</Button>
                )}
                {ts.status !== 'rejected' && (
                  <Button variant="destructive" onClick={() => handleReject(ts.id)}>Reject</Button>
                )}
              </div>
            </div>
          ))
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Input name="hours_worked" type="number" step="0.01" placeholder="Hours Worked" value={form.hours_worked || ''} onChange={handleChange} required />
            <Input name="overtime_hours" type="number" step="0.01" placeholder="Overtime Hours" value={form.overtime_hours || ''} onChange={handleChange} />
