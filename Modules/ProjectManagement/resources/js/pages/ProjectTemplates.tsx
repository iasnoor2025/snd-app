    ...
    <div className="p-6 max-w-3xl mx-auto">
      ...
      <div className="space-y-4">
        {loading ? (
          ...
          templates.map((tpl) => (
            <div key={tpl.id} className="border rounded p-4 flex flex-col md:flex-row md:items-center md:justify-between">
              <div>
                <div className="font-semibold">{tpl.name}</div>
                <div className="text-sm text-gray-500">{tpl.description}</div>
              </div>
              <div className="flex gap-2 mt-2 md:mt-0">
                <Button variant="outline" onClick={() => handleOpen(tpl)}>Edit</Button>
                <Button variant="destructive" onClick={() => handleDelete(tpl.id)}>Delete</Button>
                <Button onClick={() => { setApplyId(tpl.id); setApplyName(''); setApplyDescription(''); }}>Apply</Button>
              </div>
            </div>
          ))
        )}
      ...
      <Dialog open={!!applyId} onOpenChange={() => setApplyId(null)}>
        <DialogContent>
          ...
          />
          <div className="flex gap-2 justify-end">
            <Button type="button" variant="outline" onClick={() => setApplyId(null)}>Cancel</Button>
