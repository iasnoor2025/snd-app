...
import type { ProjectResource, ResourceFilter, ResourcePagination } from '../types/projectResources';
...
devtools(
    (set, get) => ({
        ...
        setResources: (resources) => set({ resources }, false, 'setResources'),

        addResource: (resource) => set((state) => ({ resources: [...state.resources, resource] }), false, 'addResource'),

        updateResource: (id, updatedResource) =>
            set(
                (state) => ({
                    resources: state.resources.map((resource) => (resource.id === id ? { ...resource, ...updatedResource } : resource)),
                }),
                false,
                'updateResource',
            ),
        ...
        setFilters: (newFilters) => set((state) => ({ filters: { ...state.filters, ...newFilters } }), false, 'setFilters'),

        setPagination: (pagination) => set({ pagination }, false, 'setPagination'),

        setSelectedResource: (selectedResource) => set({ selectedResource }, false, 'setSelectedResource'),
