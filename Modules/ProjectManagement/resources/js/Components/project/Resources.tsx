...
import { ArrowDown, ArrowUp, ArrowUpDown, ChevronDown, Filter, X } from 'lucide-react';
...
export default function Resources({ project }: { project: unknown }) {
    ...
    const getResourcesByType = useCallback(
        (type: ResourceType) => {
            ...
            if (filters.status.length > 0) {
                filteredResources = filteredResources.filter((resource) => filters.status.includes(resource.status));
            }
            ...
            if (sort.key) {
                filteredResources.sort((a, b) => {
                    ...
                    if (sort.key === 'worker_name') {
                        valueA = a.employee_id ? `${a.employee?.first_name || ''} ${a.employee?.last_name || ''}`.trim() : a.worker_name;
                        valueB = b.employee_id ? `${b.employee?.first_name || ''} ${b.employee?.last_name || ''}`.trim() : b.worker_name;
                    }
                    ...
                    if (valueA instanceof Date && valueB instanceof Date) {
                        return sort.direction === 'asc' ? valueA.getTime() - valueB.getTime() : valueB.getTime() - valueA.getTime();
                    }
    ...
    const getDialogTitle = useCallback(() => {
        ...
        switch (dialogType) {
            case 'manpower':
                return `${action} Manpower Resource`;
            case 'equipment':
                return `${action} Equipment Resource`;
            case 'material':
                return `${action} Material Resource`;
            case 'fuel':
                return `${action} Fuel Usage`;
            case 'expense':
    ...
    const resourceTypes = useMemo(() => ['manpower', 'equipment', 'material', 'fuel', 'expense'] as ResourceType[], []);
        ...
        () => (
            <DialogContent className="sm:max-w-[425px]">
                <DialogHeader className="flex flex-row items-center justify-between">
                    ...
                    <Button variant="ghost" size="icon" className="h-6 w-6 p-0" onClick={handleDialogClose}>
                        <X className="h-4 w-4" />
                    </Button>
                </DialogHeader>
                <DialogDescription>
                    {editingResource ? `Update the details for this ${dialogType} resource` : `Add a new ${dialogType} resource to this project`}
                </DialogDescription>
                <ResourceForm
                    type={dialogType}
