...
export default function PushPreferences() {
  ...
  return (
    <div>
      <h2 className="text-xl font-bold mb-4">{t('notifications:push.title')}</h2>
      <div className="mb-4">
      ...
      <button className="btn btn-primary mb-4" onClick={handleRegister}>{t('notifications:push.register')}</button>
      {registered && (
        <div className="mt-6">
          <h3 className="font-semibold mb-2">{t('notifications:push.send_test')}</h3>
          <div className="mb-2">
          ...
          <button className="btn btn-secondary" onClick={handleTest}>{t('notifications:push.send')}</button>
        </div>
      )}
    </div>
  );
