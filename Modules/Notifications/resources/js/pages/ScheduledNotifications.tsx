...
import { NotificationTemplate, ScheduledNotification } from '../types';
...
export default function ScheduledNotifications() {
    ...
    return (
        <div>
            <h2 className="mb-4 text-xl font-bold">{t('notifications:scheduled.title')}</h2>
            <button className="btn btn-primary mb-4" onClick={openModal}>
                {t('notifications:scheduled.add')}
            </button>
            {loading ? (
                    ...
                    <tbody>
                        {notifications.map((n) => (
                            <tr key={n.id}>
                                ...
                                <td>
                                    {n.status === 'pending' && (
                                        <button className="btn btn-sm btn-danger" onClick={() => handleCancel(n.id)}>
                                            {t('notifications:scheduled.cancel')}
                                        </button>
                                    )}
                                </td>
                            </tr>
