...
export default function Permissions({ user, directPermissions, allPermissions }: Props) {
  ...
  const handleSave = () => {
    setSaving(true);
    router.put(`/settings/users/${user.id}/permissions`, { permissions: selected }, {
      onSuccess: () => {
  ...
  return (
    <AppLayout title={`Manage Permissions: ${user.name}`} breadcrumbs={[
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <div>
              ...
              <CardDescription>
                Assign or remove direct permissions for this user.
              </CardDescription>
            </div>
          ...
          <CardContent>
            <div className="mb-4">
              <div className="font-medium">User: <span className="text-primary">{user.name}</span> ({user.email})</div>
