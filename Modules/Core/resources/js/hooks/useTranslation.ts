...
export function useTranslation(): TranslationHookReturn {
    ...
    const tl = useCallback((key: string | Record<string, string> | null | undefined, targetLocale: string) => {
    ...
    const hasTranslationForCurrent = useCallback((key: string | Record<string, string> | null | undefined, targetLocale: string) => {
        return hasTranslation(key, targetLocale);
    }, []);

    const getAllTranslationsForKey = useCallback((key: string | Record<string, string> | null | undefined) => {
    ...
    const validateRequired = useCallback((key: string | Record<string, string> | null | undefined, requiredLocales: string[]) => {
        return validateRequiredTranslations(key, requiredLocales);
    }, []);

    // Locale management functions
    const handleSwitchLocale = useCallback((targetLocale: string, preserveState: boolean = true) => {
        switchLocale(targetLocale, preserveState);
    }, []);

    const getDisplayName = useCallback((targetLocale: string) => {
