...
function findModulePagesByPattern(moduleNamePattern: string, pageNamePattern: string): string[] {
    return Object.keys(modulePages).filter(
        (path) => path.includes(moduleNamePattern) && (path.includes(pageNamePattern) || path.toLowerCase().includes(pageNamePattern.toLowerCase())),
    );
}
        ...
        try {
            ...
            try {
                ...
                for (const key in modulePages) {
                    ...
                    if (keyNoExt.endsWith(`/${normalizedName}`) || keyNoExt.toLowerCase().endsWith(`/${normalizedName.toLowerCase()}`)) {
                        return await modulePages[key]();
                    }
                }
