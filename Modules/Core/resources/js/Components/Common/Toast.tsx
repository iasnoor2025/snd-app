            ...
            >
                <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 space-y-1">
                        {title && <ToastTitle className={cn('leading-none font-medium tracking-tight', compact && 'text-sm')}>{title}</ToastTitle>}
                        {description && <ToastDescription className={cn('text-sm opacity-90', compact && 'text-xs')}>{description}</ToastDescription>}
                        {actions && <div className={cn('mt-2 flex items-center gap-2', compact && 'mt-1 gap-1')}>{actions}</div>}
                    </div>
                    <ToastClose
                        onClick={onClose}
                        className={cn('opacity-70 transition-opacity hover:opacity-100', compact && 'h-4 w-4')}
                        aria-label="Close toast"
                    />
                </div>
                {showProgress && duration > 0 && (
                    <div
                        className="bg-opacity-20 absolute bottom-0 left-0 h-1 bg-white"
