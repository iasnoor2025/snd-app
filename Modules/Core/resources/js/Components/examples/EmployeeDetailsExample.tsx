...
const EmployeeDetailsExample: React.FC<{ employee: any }> = ({ employee }) => {
    ...
    return (
        <div className={`space-y-4 ${isRTL ? 'text-right' : 'text-left'}`}>
            <div>
                <h3 className="mb-3 text-sm font-medium text-muted-foreground">{t('employees:contact_info')}</h3>
            ...
            <div>
                ...
                <dl className="space-y-2">
                    ...
                    {!employee.iqama_number && !employee.passport_number && !employee.date_of_birth && (
                        <p className="text-sm text-muted-foreground italic">{t('employees:no_identification_info')}</p>
                    )}
                </dl>
            </div>

            <div>
                <h3 className="mb-3 text-sm font-medium text-muted-foreground">{t('employees:licenses')}</h3>
                ...
                employee.spsp_license_number ? (
                    ...
                    <p className="text-sm text-muted-foreground italic">{t('employees:no_licenses_info')}</p>
                )}
            </div>
        </div>
    );
