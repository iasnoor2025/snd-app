  ...
  return (
    <SidebarContext.Provider value={{ collapsible }}>
      <aside
        ...
        className={cn(
          "group/sidebar-wrapper flex h-full flex-col transition-[width] ease-linear",
          className
        )}
        {...props}
      >
...
export function SidebarMenuButton({ children, className, size = "md", asChild = false, ...props }: React.ButtonHTMLAttributes<HTMLButtonElement> & { size?: "sm" | "md" | "lg"; asChild?: boolean }) {
  const sizes = {
  ...
  return (
    <Comp
      className={cn(
        "flex w-full items-center gap-2 rounded transition-colors hover:bg-accent focus:outline-none focus:ring-2 focus:ring-primary",
        sizes[size],
        className
      )}
      {...props}
...
export function SidebarTrigger({ className, ...props }: React.ButtonHTMLAttributes<HTMLButtonElement>) {
  return (
    <button
      ...
      className={cn(
        "inline-flex items-center justify-center rounded p-2 text-muted-foreground hover:bg-accent focus:outline-none focus:ring-2 focus:ring-primary",
        className
      )}
      {...props}
    >
...
export function SidebarGroupLabel({ children, className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return (
    <div className={cn("px-3 py-1 text-xs font-semibold uppercase text-muted-foreground", className)} {...props}>
      {children}
    </div>
  );
}
...
export function SidebarMenuSubButton({ children, className, ...props }: React.ButtonHTMLAttributes<HTMLButtonElement>) {
  return (
    <button
      className={cn(
        "flex w-full items-center gap-2 rounded px-3 py-2 text-base transition-colors hover:bg-accent focus:outline-none focus:ring-2 focus:ring-primary",
        className
      )}
      {...props}
    >
