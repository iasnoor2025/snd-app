...
function Calendar({
  ...
  // Enhanced onSelect handler with error handling
  const safeOnSelect = React.useCallback((date: Date | undefined) => {
    if (process.env.NODE_ENV === 'development') {
      console.log('Calendar safeOnSelect called with:', {
        date,
    ...
    try {
      if (onSelect && typeof onSelect === 'function') {
        ...
        console.warn('Calendar onSelect is not a function:', {
          onSelect,
      ...
      console.error('Error in Calendar onSelect:', {
        error,
        date,
        onSelect,
  ...
  }, [onSelect, isArabic, i18n.language])

  // If Arabic is selected and Persian calendar is available, use Persian calendar
  if (isArabic && PersianDatePicker) {
    const selectedDate = (props as any).selected || (props as any).value
    return (
      <div className={cn("p-3", className)}>
        <PersianDatePicker
          input={false}
          selectedDay={selectedDate}
          onSelectDay={safeOnSelect}
  ...
  // Enhanced formatters for better Arabic support
  const enhancedFormatters = React.useMemo(() => {
    const baseFormatters = {
      formatMonthDropdown: (date: Date) =>
        date.toLocaleString(isArabic ? "ar-SA" : "default", { month: "short" }),
      ...formatters,
    }
