...
class UpdateGeofenceZoneRequest extends FormRequest
{
    /**
    ...
    public function authorize(): bool
    {
        ...
        return auth()->check() && auth()->user()->can('update', $geofence);
    }
    ...
    public function rules(): array
    {
        ...
        return [
            'name' => [
                ...
                'string',
                'max:255',
                Rule::unique('geofence_zones', 'name')->ignore($geofence->id)
            ],
            ...
            'radius_meters' => 'sometimes|required|integer|min:1|max:50000',
            'polygon_coordinates' => 'sometimes|nullable|array|min:3',
            ...
            'strict_enforcement' => 'sometimes|boolean',
            'buffer_meters' => 'sometimes|nullable|integer|min:0|max:1000',
            'time_restrictions' => 'sometimes|nullable|array',
            'time_restrictions.enabled' => 'sometimes|boolean',
            ...
            'time_restrictions.days_of_week' => 'sometimes|nullable|array',
    ...
    {
        // Convert boolean strings to actual booleans for fields that are present
        $booleanFields = [
            'is_active',
            'strict_enforcement',
            'monitoring_enabled',
        ...
        // Set buffer_meters to 0 if not provided but other fields are being updated
        if (!$this->has('buffer_meters') && $this->hasAny(['radius_meters', 'center_latitude', 'center_longitude'])) {
            $this->merge(['buffer_meters' => 0]);
        }
    }
    ...
    public function withValidator($validator): void
    {
        $validator->after(function ($validator) {
            ...
            if ($this->has('project_id') && $this->project_id && !\Modules\Core\Domain\Models\Project::find($this->project_id)) {
            ...
            // Custom validation: ensure at least one coordinate method is provided
            if ($this->hasAny(['center_latitude', 'center_longitude', 'radius_meters', 'polygon_coordinates'])) {
                $geofence = $this->route('geofence');

                // If updating coordinates, ensure we have complete coordinate data
                $hasCenter = ($this->has('center_latitude') && $this->has('center_longitude')) ||
                           ($geofence->center_latitude && $geofence->center_longitude);

                $hasRadius = $this->has('radius_meters') || $geofence->radius_meters;

                $hasPolygon = ($this->has('polygon_coordinates') && !empty($this->polygon_coordinates)) ||
                            (!empty($geofence->polygon_coordinates));

                if (!$hasCenter && !$hasPolygon) {
                    $validator->errors()->add('coordinates', 'Either center coordinates or polygon coordinates must be provided.');
                }

                if ($hasCenter && !$hasRadius && !$hasPolygon) {
                    $validator->errors()->add('radius_meters', 'Radius is required when using center coordinates without polygon.');
                }
            }
        });
    }
    ...
    /**
     * Get the validated data with only the fields that were actually provided.
     */
    public function validatedOnly(): array
    {
        ...
        return array_intersect_key($validated, array_flip($provided));
    }
}
