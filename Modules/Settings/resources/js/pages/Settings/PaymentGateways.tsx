...
export default function PaymentGateways() {
    ...
    return (
        <div>
            <h2 className="mb-4 text-xl font-bold">{t('settings:payment_gateways.title')}</h2>
            <button className="btn btn-primary mb-4" onClick={() => openModal()}>
                {t('settings:payment_gateways.add')}
            </button>
            {loading ? (
                    ...
                    <tbody>
                        {settings.map((s) => (
                            <tr key={s.id}>
                                ...
                                <td>
                                    <button className="btn btn-sm btn-secondary mr-2" onClick={() => openModal(s)}>
                                        {t('common:edit')}
                                    </button>
                    ...
                    <div className="modal-box">
                        <form onSubmit={handleSubmit}>
                            ...
                            <div className="mb-2">
                                ...
                                <textarea
                                    name="endpoints"
                                    value={JSON.stringify(form.endpoints || {}, null, 2)}
                                    onChange={handleJsonChange}
                                    className="input input-bordered w-full"
                                    rows={3}
                                    required
