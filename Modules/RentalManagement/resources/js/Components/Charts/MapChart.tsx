    ...
    return (
        <div style={{ height: `${height}px`, width: '100%' }}>
            <MapContainer center={[center.lat, center.lng]} zoom={13} style={{ height: '100%', width: '100%' }}>
                ...
                {data.map((point) => (
                    <Marker key={`${point.equipment_id}-${point.latitude}-${point.longitude}`} position={[point.latitude, point.longitude]}>
                        <Popup>
                            <div>
                                <h3 className="font-medium">{point.name}</h3>
                                <p className="text-sm">Equipment ID: {point.equipment_id}</p>
                                {point.last_update && <p className="text-sm text-gray-500">Last update: {new Date(point.last_update)}</p>}
                            </div>
                        </Popup>
                    </Marker>
                ))}
