import { cn, usePermission } from '@/Core';
...
// Icons
import { AlertCircle, ArrowRight, CalendarCheck, CheckCircle, CircleDashed, ClipboardCheck, Clock, FileText, Receipt, Truck } from 'lucide-react';
...
export default function RentalWorkflowStatus({ rental, nextPossibleStates = [], className = '' }: RentalWorkflowStatusProps) {
    ...
    const possibleNextSteps =
        nextPossibleStates && Array.isArray(nextPossibleStates) ? workflowSteps.filter((step) => nextPossibleStates.includes(step.id)) : [];

    const confirmAction = (title: string, description: string, confirmText: string, action: () => void) => {
        setDialogAction({
            title,
            description,
            confirmText,
    ...
    const createActionHandler = (statusId: string, title: string, description: string, routeName: string, confirmText: string = 'Confirm') => {
        return () => {
            confirmAction(title, description, confirmText, () => {
                // Actually send the POST request to the backend
                try {
                    Inertia.post(
                        route(routeName, rental.id),
                        {},
                        {
                            preserveScroll: true,
                            preserveState: true,
        ...
        <Card className={className}>
            <CardHeader>
                <CardTitle className="flex items-center justify-between">
                    ...
                    <Badge variant="outline" className={`px-3 py-1 text-sm ${currentStep.color}`}>
            ...
            <CardContent>
                <div className="space-y-6">
                    ...
                    <div className="relative">
                        {/* Progress line */}
                        <div className="absolute top-4 left-4 h-full w-0.5 bg-gray-200" />
                        <div
                            className="absolute top-4 left-4 h-full w-0.5 bg-primary transition-all duration-500"
                            style={{
                                            ...
                                            <TooltipTrigger asChild>
                                                <div
                                                    className={cn(
                                                        ...
                                                        isCurrent && 'rounded-md bg-gray-50 p-2 shadow-sm',
                                                    )}
                                                >
                                                    {/* Step indicator */}
                                                    <div
                                                    ...
                                                    <div className="flex-1">
                                                        <h3 className={cn('text-sm font-medium', isCurrent && 'text-primary')}>
                                                            {step.name}
                                                            {isCurrent && (
                                                                <span className="ml-2 inline-block rounded bg-gray-200 px-2 py-0.5 align-middle text-xs font-semibold text-gray-700">
                                                                    Current
                                                                </span>
                                                            )}
                                                        </h3>
                                                        ...
                                                        {isCurrent &&
                                                            ...
                                                            (() => {
                                                                ...
                                                                return (
                                                                    ...
                                                                    >
                                                                        {Icon && <Icon className="mr-2 inline-block h-4 w-4 align-text-bottom" />}
