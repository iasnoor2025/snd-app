...
import { AlertCircle, CalendarIcon, Loader2, MoreHorizontal, UserX } from 'lucide-react';
...
const formSchema = z
    .object({
        ...
        hours_used: z.coerce.number().min(0.5, 'Minimum 0.5 hours required').max(24, 'Maximum 24 hours allowed'),
...
export default function TimesheetForm({
    ...
    useEffect(() => {
        if (isEditing && timesheet) {
            ...
            const formattedDate = timesheet.date ? format(new Date(timesheet.date), 'yyyy-MM-dd') : format(new Date(), 'yyyy-MM-dd');
            ...
            const currentRentalItem = validRentalItems.find((item) => item.id.toString() === timesheet.rental_item_id?.toString());
    ...
    const hasAdminAccess = auth?.user?.role === 'admin' || auth?.user?.role === 'manager' || auth?.user?.role === 'accountant';
    ...
    const handleTimesheetAction = (action: 'approve' | 'reject' | 'void' | 'edit' | 'delete') => {
        ...
        if (window.confirm(confirmMessages[action])) {
            router.put(
                route('rental-timesheets.update-status', timesheet.id),
                {
    ...
    const renderEquipmentSelection = () => {
        ...
        return (
            <>
                <Select value={form.data.rental_item_id?.toString() || ''} onValueChange={handleRentalItemChange}>
                    <SelectTrigger>
                            ...
                            {validRentalItems.map((item) => (
                                <SelectItem key={item.id} value={item.id.toString()}>
                                    ...
                                    {item.equipment?.serial_number && (
                                        <span className="ml-2 text-muted-foreground">(SN: {item.equipment.serial_number})</span>
                                    )}
                                </SelectItem>
                            ))}
                        </SelectGroup>
                ...
                <Card>
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                        <div>
                            ...
                            <CardDescription>{isEditing ? 'Update' : 'Create'} timesheet for tracking equipment usage</CardDescription>
                        </div>
                        {isEditing && hasAdminAccess && (
                            <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                    ...
                                    {isCompleted ? (
                                        <>
                                            <DropdownMenuItem onClick={() => handleTimesheetAction('approve')}>Approve Timesheet</DropdownMenuItem>
                                            <DropdownMenuItem onClick={() => handleTimesheetAction('reject')}>Reject Timesheet</DropdownMenuItem>
                                            <DropdownMenuItem onClick={() => handleTimesheetAction('void')} className="text-destructive">
                                                Void Timesheet
                                            </DropdownMenuItem>
                                        </>
                                    ) : null}
                                    <DropdownMenuItem onClick={() => handleTimesheetAction('edit')}>Edit Timesheet</DropdownMenuItem>
                    ...
                    <CardContent className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                            ...
                            <div className="space-y-2">
                                ...
                                {form.errors.operator_id && <p className="mt-1 text-xs text-destructive">{form.errors.operator_id}</p>}
                            </div>
                        </div>
                        ...
                        {form.data.operator_id !== 'none' && form.data.operator_id && (
                            <div className="flex items-center space-x-2 pt-6">
                                <Checkbox id="operator_absent" checked={operatorAbsent} onCheckedChange={handleOperatorAbsentChange} />
                                <div className="grid gap-1.5">
                                    <Label htmlFor="operator_absent" className="flex cursor-pointer items-center gap-1 text-base">
                                        {t('mark_operator_as_absent')}
                                    </Label>
                                    <p className="text-sm text-muted-foreground">Hours will be set to 0 and no time entries will be recorded</p>
                                </div>
                            </div>
                        )}
                        ...
                        <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
                            ...
                            <div className="space-y-2">
                                <Label htmlFor="start_time">
                                    {t('lbl_start_time')} <span className="text-destructive">*</span>
                                </Label>
                                ...
                                {form.errors.start_time && <p className="mt-1 text-xs text-destructive">{form.errors.start_time}</p>}
                            </div>
                            ...
                            <div className="space-y-2">
                                ...
                                {form.errors.end_time && <p className="mt-1 text-xs text-destructive">{form.errors.end_time}</p>}
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
                            <div className="space-y-2">
                                <Label htmlFor="hours_used">
                                    {t('lbl_hours_used')} <span className="text-destructive">*</span>
                                </Label>
                                <Input
                                    id="hours_used"
                                    type="number"
                                ...
                                />
                                {form.errors.hours_used && <p className="mt-1 text-xs text-destructive">{form.errors.hours_used}</p>}
                            </div>
                        </div>
