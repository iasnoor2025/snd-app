...
import { AlertCircle, Calendar, CheckCircle, Clock, FileText, PackageCheck, Receipt, Truck } from 'lucide-react';
            ...
            {timelineSteps.map((step, index) => (
                <div key={index} className="relative">
                    ...
                    {index < timelineSteps.length - 1 && (
                        <div className={`absolute top-10 bottom-0 left-5 w-0.5 ${step.status === 'completed' ? 'bg-primary' : 'bg-muted'}`} />
                    )}
                    ...
                    <div className="flex gap-4">
                        <div
                            className={`flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full ${
                                step.status === 'completed' ? 'bg-primary/10' : step.status === 'current' ? 'animate-pulse bg-red-100' : 'bg-muted'
                            }`}
                        >
                            {step.icon}
                        </div>
