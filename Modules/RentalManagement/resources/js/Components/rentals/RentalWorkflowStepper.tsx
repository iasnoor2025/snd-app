...
export function RentalWorkflowStepper({ currentStatus, className }: RentalWorkflowStepperProps) {
    ...
    const currentIndex = workflowSteps.findIndex((step) => step.status === validStatus);
    ...
    return (
        <div className={cn('w-full', className)}>
            <div className="relative">
                ...
                <div
                    className="absolute top-0 left-4 h-full w-0.5 bg-primary transition-all duration-500"
                    style={{
                        ...
                        return (
                            <div
                                ...
                                className={cn(
                                    ...
                                    isCurrent && 'rounded-md bg-gray-50 p-2 shadow-sm', // Highlight current step
                                )}
                            >
                                {/* Step indicator */}
                                <div
                                ...
                                <div className="flex-1">
                                    <h3
                                        className={cn(
                                            ...
                                            isCurrent && 'font-bold text-primary', // Make current text bolder
                                            isUpcoming && 'text-gray-500',
                                        )}
                                    >
                                        {step.label || 'Unknown Step'}
                                        {isCurrent && (
                                            <span className="ml-2 inline-block rounded-full bg-primary px-2 py-0.5 text-xs text-white">Current</span>
                                        )}
                                    </h3>
                                    <p className={cn('text-sm', isCurrent ? 'text-gray-700' : 'text-gray-500')}>
