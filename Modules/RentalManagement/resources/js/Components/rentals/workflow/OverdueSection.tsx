...
import { AlertCircle, AlertTriangle, Printer, Send } from 'lucide-react';
...
export default function OverdueSection({ rental, rentalItems, invoices, permissions }: OverdueSectionProps) {
    ...
    const getPaymentProgress = () => {
        ...
        const paidAmount = invoices.data.filter((inv) => inv.status === 'paid').reduce((sum, inv) => sum + (inv.amount || 0), 0);
    ...
    return (
        <div className="space-y-4">
            ...
            <Alert variant="destructive" className="animate-pulse">
                ...
                <AlertDescription>
                    This rental has exceeded its expected end date by {getDaysOverdue()} days. Immediate action is required.
                </AlertDescription>
            </Alert>
            ...
            <Card className="border-red-200">
                <CardHeader className="border-b border-red-100 bg-red-50">
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <CardDescription>The rental period has ended but equipment hasn't been returned</CardDescription>
                        </div>
                        <Badge variant="destructive">{getDaysOverdue()} Days Overdue</Badge>
                    </div>
                </CardHeader>
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="space-y-2">
                            <div className="flex items-center justify-between">
                                ...
                                <span className={`text-sm font-medium ${getPaymentProgress() === 100 ? 'text-green-600' : 'text-red-600'}`}>
                            ...
                            <div className="space-y-3">
                                <div>
                                    ...
                                    <p className="font-medium text-red-600">
                                        {formatCurrency(
                                            invoices.data?.filter((inv) => inv.status !== 'paid').reduce((sum, inv) => sum + (inv.amount || 0), 0) ||
                                                0,
                                        )}
                                    </p>
                                </div>
                                <div>
                                    ...
                                    <p className="font-medium text-red-600">
                                        {formatCurrency(rental.late_fees || getDaysOverdue() * (rental.daily_late_fee || rental.daily_rate || 25))}
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex gap-2 pt-2">
                            <Button variant="default" className="flex-1" onClick={handleSendReminder} disabled={isSendingReminder}>
                                <Send className="mr-2 h-4 w-4" />
                                Send Payment Reminder
                            </Button>

                            <Button variant="outline" className="flex-1" onClick={handlePrintInvoice} disabled={isPrintingInvoice}>
                ...
                <CardFooter className="border-t border-red-100 bg-red-50 text-sm text-red-700">
