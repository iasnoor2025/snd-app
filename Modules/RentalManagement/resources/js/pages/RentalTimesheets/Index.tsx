import { AppLayout, formatDate } from '@/Core';
...
// Icons
import { cn, formatCurrency, Pagination } from '@/Core';
import { formatDateMedium } from '@/Core/utils/dateFormatter';
import { format } from 'date-fns';
import { CalendarIcon, Check, ChevronRight, Eye, Filter, Home, MoreHorizontal, Pencil, Plus, RotateCw, Trash, User, UserX } from 'lucide-react';
...
export default function Index({ auth, timesheets, filters = {} }: Props) {
    ...
    const renderOperator = (timesheet: RentalTimesheet) => {
        ...
        return <span className="text-sm text-muted-foreground">{t('not_assigned')}</span>;
    };
                    ...
                    <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                            <DropdownMenuItem asChild>
                                <Link href={route('rental-timesheets.show', timesheet.id)}>View</Link>
                            </DropdownMenuItem>
                            {canEdit && timesheet.status === 'pending' && (
                                <DropdownMenuItem asChild>
                                    <Link href={route('rental-timesheets.edit', timesheet.id)}>Edit</Link>
                                </DropdownMenuItem>
                            )}
                            {canDelete && (
                                <DropdownMenuItem
                                    onClick={() => {
                                        deleteId = timesheet.id;
                                        handleDelete();
                                    }}
                                    ...
                                    <Popover>
                                        <PopoverTrigger asChild>
                                            ...
                                            >
                                                ...
                                                {filterValues.start_date ? format(filterValues.start_date, 'PPP') : <span>{t('pick_a_date')}</span>}
                                            </Button>
                                        </PopoverTrigger>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Popover>
                                        <PopoverTrigger asChild>
                                            ...
                                            >
                                                <CalendarIcon className="mr-2 h-4 w-4" />
                                                {filterValues.end_date ? format(filterValues.end_date, 'PPP') : <span>{t('pick_a_date')}</span>}
                                            </Button>
                                        </PopoverTrigger>
                                        <PopoverContent className="w-auto p-0">
                                            <Calendar
                        ...
                        {selectedIds.length > 0 && (
                            <div className="mb-4 flex items-center justify-between rounded-md bg-muted p-2">
                                ...
                                <div className="flex gap-2">
                                    ...
                                    <Button variant="default" size="sm" onClick={() => setIsCompleteDialogOpen(true)}>
                                    ...
                                    {timesheets.data.length === 0 ? (
                                        ...
                                        timesheets.data.map((timesheet) => (
                                            <TableRow key={timesheet.id}>
                                                ...
                                                <TableCell>{timesheet.rental?.rental_number || 'N/A'}</TableCell>
                                                <TableCell>{timesheet.rentalItem?.equipment?.name || 'N/A'}</TableCell>
                                                <TableCell>{renderOperator(timesheet)}</TableCell>
                                                <TableCell>
            ...
            <Dialog open={isCompleteDialogOpen} onOpenChange={setIsCompleteDialogOpen}>
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Are you sure you want to mark {selectedIds.length} timesheet(s) as completed? This action cannot be undone.
                        </DialogDescription>
                    </DialogHeader>
                    <DialogFooter>
                        ...
                        <Button onClick={handleBulkComplete}>Complete Timesheets</Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
        </AppLayout>
