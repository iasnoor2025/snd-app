import { AppLayout, ErrorAlert, ErrorBoundary, usePermission } from '@/Core';
...
export default function ForRental({ auth, rental, timesheets, debug }: Props) {
    ...
    const totalHours = timesheets.reduce((sum, timesheet) => sum + (timesheet.hours_used ? parseFloat(String(timesheet.hours_used)) : 0), 0);
    ...
    const getFilteredTimesheets = () => {
        ...
        if (search.trim() !== '') {
            filtered = filtered.filter((timesheet) => {
                ...
                return equipmentName.includes(searchLower) || operatorName.includes(searchLower) || date.includes(searchLower);
            });
        }
    ...
    const completeSelectedTimesheets = () => {
        ...
        router.post(
            route('rental-timesheets.bulk-complete'),
            {
    ...
    const bulkDeleteTimesheets = () => {
        ...
        router.post(
            route('rental-timesheets.bulk-delete'),
            {
                timesheet_ids: selectedIds,
            },
            {
    ...
    const completeTimesheet = (id: number) => {
        router.put(
            route('rental-timesheets.complete', id),
            {},
            {
                onSuccess: () => {
    ...
    const checkMissingTimesheets = () => {
        router.get(
            route('rental-timesheets.check-missing', rental.id),
            {},
            {
    ...
    const confirmMarkOperatorAbsent = () => {
        if (timesheetToMarkAbsent) {
            router.put(
                route('rental-timesheets.mark-absent', timesheetToMarkAbsent),
                {},
                {
                    onSuccess: () => {
    ...
    const getOperatorStatusBadge = (timesheet: any) => {
        if (timesheet.operator_absent) {
            return (
                <Badge variant="outline" className="mt-1 border-red-200 bg-red-50 text-red-600">
                    Absent
                </Badge>
            );
        }
    ...
    const statCards = (
        <div className="mb-8 grid grid-cols-1 gap-4 md:grid-cols-4">
            <div className="flex flex-col items-center justify-center gap-2 rounded-md bg-muted/20 p-4">
                <h3 className="text-sm font-medium text-muted-foreground">{t('total_hours_used')}</h3>
                <p className="text-2xl font-bold">
                    {typeof totalHours === 'number' ? totalHours.toFixed(1) : parseFloat(String(totalHours || 0)).toFixed(1)}
                </p>
            </div>
                ...
                <div className="container mx-auto space-y-6 py-6">
                    ...
                    <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
                        ...
                        <div className="flex flex-wrap gap-2">
                            ...
                            <Button variant="outline" size="sm" onClick={() => setIsMissingDatesDialogOpen(true)}>
                        ...
                        <CardContent className="p-6">
                            <div className="flex flex-col justify-between gap-4 md:flex-row">
                                ...
                                <div className="flex items-center gap-2">
                                    <Badge variant={rental.has_timesheet ? 'default' : 'outline'}>{timesheets.length} Timesheets</Badge>
                    ...
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>Track equipment usage hours for this rental</CardDescription>
                        ...
                        <CardContent>
                            {timesheets.length === 0 ? (
                                <div className="rounded-md border py-16 text-center">
                                    ...
                                    <p className="mb-4 text-muted-foreground">No timesheets have been created for this rental yet.</p>
                                    <Button asChild>
                                        <Link href={route('rental-timesheets.create', { rental_id: rental.id })}>
                                            <Plus className="mr-2 h-4 w-4" />
                                ...
                                <div className="rounded-md border py-16 text-center">
                                    <Search className="mx-auto mb-4 h-12 w-12 text-muted-foreground opacity-20" />
                                    <p className="mb-4 text-muted-foreground">No timesheets match your search criteria.</p>
                                    <Button
                                        variant="outline"
                                        onClick={() => {
                                            setStatusFilter('all');
                                                ...
                                                <TableRow key={timesheet.id}>
                                                    ...
                                                    <TableCell className="font-medium">
                                                        {timesheet.rentalItem?.equipment ? (
                                                            ...
                                                            <div className="flex flex-col">
                                                                <span className="font-medium">{timesheet.equipment.name || 'Unnamed Equipment'}</span>
                                                                        ...
                                                                        <TooltipContent className="max-w-sm">
                                                                            {timesheet.rentalItem ? (
                                                                                <>
                                                                                    ...
                                                                                    <p className="mt-1 text-xs">
                                                                                        Equipment ID:{' '}
                                                                                        {timesheet.rentalItem.equipment_id
                                                    ...
                                                    <TableCell>{format(new Date(timesheet.date), 'EEE, MMM dd, yyyy')}</TableCell>
                                                    <TableCell>
                                                            ...
                                                            <span className="text-xs">
                                                                {timesheet.start_time && timesheet.end_time ? (
                                                                    <>
                                                                        ...
                                                                        <p className="mt-1 text-muted-foreground">{timesheet.hours_used} hours</p>
                                                                        <div className="mt-1 w-full">
                                                                            <Progress
                                                                                value={Math.min(
                                                                                    ((timesheet.hours_used
                                                                                        ? parseFloat(String(timesheet.hours_used))
                                                                                        : 0) /
                                                                                        8) *
                                                                                        100,
                                                                                    100,
                                                        ...
                                                        {timesheet.operator_absent ? (
                                                            ...
                                                            <div className="flex flex-col">
                                                                <span className="flex items-center gap-1 font-medium">
                                                                    ...
                                                                    {(timesheet.operator as any).first_name || ''}{' '}
                                                                    {(timesheet.operator as any).last_name || ''}
                                                                </span>
                                                            ...
                                                            <div className="flex flex-col">
                                                                <span className="flex items-center gap-1 font-medium text-amber-600">
                                                                    <User className="h-3 w-3 text-muted-foreground" />
                                                                    {timesheet.rentalItem.operator.first_name}{' '}
                                                                    {timesheet.rentalItem.operator.last_name}
                                                                    <TooltipProvider>
                                                                        <Tooltip>
                                                                            <TooltipTrigger>
                                                    ...
                                                    <TableCell className="text-right">
                                                        <div className="flex justify-end gap-2">
                                                            ...
                                                            {timesheet.status !== ('completed' as boolean) && (
                                                                <Button variant="ghost" size="icon" onClick={() => completeTimesheet(timesheet.id)}>
                            ...
                            {/* Pagination controls */}
                            {filteredTimesheets.length > 0 && (
                                <div className="mt-4 flex flex-col items-center justify-between gap-4 border-t pt-4 text-sm sm:flex-row">
                                    <div className="text-muted-foreground">
                                        Showing{' '}
                                        <span className="font-medium">{Math.min((currentPage - 1) * pageSize + 1, filteredTimesheets.length)}</span>{' '}
                                        to <span className="font-medium">{Math.min(currentPage * pageSize, filteredTimesheets.length)}</span> of{' '}
                                        <span className="font-medium">{filteredTimesheets.length}</span> timesheets
                                    </div>

                                    <div className="flex items-center gap-2">
                                        <div className="flex items-center gap-2">
                                            <span className="whitespace-nowrap text-muted-foreground">Items per page:</span>
                                            <Select
                ...
                <Dialog open={isCompleteDialogOpen} onOpenChange={setIsCompleteDialogOpen}>
                    <DialogContent>
                        <DialogHeader>
                            ...
                            <DialogDescription>
                                Are you sure you want to mark {selectedIds.length} timesheets as completed? This action cannot be undone.
                            </DialogDescription>
                        </DialogHeader>
                        <DialogFooter>
                            ...
                            <Button variant="default" onClick={completeSelectedTimesheets} className="bg-green-600 hover:bg-green-700">
                                <Check className="mr-2 h-4 w-4" />
                                Complete All
                            </Button>
                        </DialogFooter>
                ...
                <Dialog open={isMissingDatesDialogOpen} onOpenChange={setIsMissingDatesDialogOpen}>
                    <DialogContent>
                        <DialogHeader>
                            ...
                            <DialogDescription>
                                ...
                                <p className="mt-2 text-sm">
                                    Start Date:{' '}
                                    <span className="font-medium">
                                        {rental.start_date ? format(new Date(rental.start_date), 'MMM dd, yyyy') : 'Not set'}
                                    </span>
                                </p>
                            </DialogDescription>
                        </DialogHeader>
                        <DialogFooter>
                            ...
                            <Button variant="default" onClick={checkMissingTimesheets}>
                                <Clock className="mr-2 h-4 w-4" />
                                Check Missing Dates
                            </Button>
                        </DialogFooter>
                ...
                <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
                    <DialogContent>
                        ...
                        <DialogFooter>
                            ...
                            <Button variant="destructive" onClick={confirmDeleteTimesheet}>
                                <Trash className="mr-2 h-4 w-4" />
                ...
                <Dialog open={isBulkDeleteDialogOpen} onOpenChange={setIsBulkDeleteDialogOpen}>
                    <DialogContent>
                        <DialogHeader>
                            ...
                            <DialogDescription>
                                Are you sure you want to delete {selectedIds.length} timesheets? This action cannot be undone.
                            </DialogDescription>
                        </DialogHeader>
                        <DialogFooter>
                            ...
                            <Button variant="destructive" onClick={bulkDeleteTimesheets}>
                                <Trash className="mr-2 h-4 w-4" />
                ...
                <Dialog open={isAbsentDialogOpen} onOpenChange={setIsAbsentDialogOpen}>
                    <DialogContent>
                        <DialogHeader>
                            ...
                            <DialogDescription>
                                Are you sure you want to mark the operator as absent for this timesheet? This will record that the equipment was not
                                used on this date due to operator absence.
                            </DialogDescription>
                        </DialogHeader>
                        <DialogFooter>
                            ...
                            <Button variant="default" onClick={confirmMarkOperatorAbsent} className="bg-red-600 hover:bg-red-700">
                                    ...
                                    <TableBody>
                                        {debug.problemTimesheets.map((problem) => (
                                            <TableRow key={problem.id} className="bg-amber-50">
                                                ...
                                                <TableCell>
                                                    {problem.rental_item_exists !== undefined ? (problem.rental_item_exists ? 'Yes' : 'No') : 'N/A'}
                                                </TableCell>
                                                <TableCell>{problem.equipment_id || 'N/A'}</TableCell>
                                                <TableCell>
                                                    {problem.equipment_exists !== undefined ? (problem.equipment_exists ? 'Yes' : 'No') : 'N/A'}
                                                </TableCell>
                                            </TableRow>
                                        ))}
                                    </TableBody>
