...
// Icons
import { ArrowDown, ArrowUp, BarChart3, CalendarIcon, Download, FileText, List, Loader2, Printer, RefreshCw } from 'lucide-react';
...
export default function Report({ auth, rentals, customers, filters }: Props) {
    ...
    const [startDate, setStartDate] = useState<Date | undefined>(filters.start_date ? parseISO(filters.start_date) : undefined);
    const [endDate, setEndDate] = useState<Date | undefined>(filters.end_date ? parseISO(filters.end_date) : undefined);
    const [status, setStatus] = useState<string | undefined>(filters.status || undefined);
    const [clientId, setClientId] = useState<string | undefined>(filters.client_id ? String(filters.client_id) : undefined);
    ...
    const debouncedApplyFilters = (
        ...
        filterType: 'none' | 'start-date' | 'end-date' | 'status' | 'customer' | 'filter',
    ) => {
    ...
    const calculateTotalAmount = () => {
        try {
            ...
            return rentals.data.reduce((sum, rental) => {
                ...
                const amount = rental.total_amount !== undefined && rental.total_amount !== null ? Number(rental.total_amount) : 0;
    ...
    const averageDuration =
        rentals?.data?.length > 0
            ? rentals.data.reduce((sum, rental) => {
                  const days = rental.duration_days !== undefined && rental.duration_days !== null ? Number(rental.duration_days) : 0;
                  return sum + (isNaN(days) ? 0 : days);
              }, 0) / rentals.data.length
            : 0;

    // Count by status
    const statusCounts = rentals?.data
        ? rentals.data.reduce((counts: Record<string, number>, rental) => {
                ...
                <div className="container mx-auto print:px-0">
                    ...
                    <div className="mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                        <div>
                            ...
                            <p className="text-muted-foreground">{getFilterSummary()}</p>
                        </div>
                        <div className="flex items-center gap-2 print:hidden">
                            <Button variant="outline" onClick={printReport} disabled={!rentals?.data?.length || loading} size="sm">
                                <Printer className="mr-2 h-4 w-4" />
                                Print
                            </Button>
                            <Button variant="outline" onClick={exportReport} disabled={!rentals?.data?.length || loading} size="sm">
                    ...
                    <Card className="mb-6 print:hidden">
                        ...
                        <CardContent>
                            <div className="flex flex-wrap items-end gap-3 lg:gap-4">
                                ...
                                <div className="w-full sm:w-auto">
                                    ...
                                    <Popover>
                                        <PopoverTrigger asChild>
                                            <Button variant="outline" className="w-full justify-start pl-3 text-left font-normal sm:w-[160px]">
                                                {startDate ? (
                                ...
                                {/* Generate Button */}
                                <Button onClick={generateReport} disabled={loadingOp !== 'none'} className="mt-2 sm:mt-0">
                                    {loadingOp === 'filter' ? (
                                        <>
                    ...
                    {loading || loadingOp !== 'none' ? (
                                        ...
                                        <Card key={i}>
                                            <CardContent className="pt-6">
                                                <div className="space-y-2">
                                                    ...
                                                    <Skeleton className="mx-auto h-8 w-[80px]" />
                                                </div>
                                            </CardContent>
                                        </Card>
                                    ))}
                        ...
                        <>
                                            ...
                                            <CardContent>
                                                <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4">
                                                    {Object.entries(statusCounts).map(([status, count]) => (
                                                        <div key={status} className="flex flex-col items-center rounded-lg bg-muted/30 p-4">
                                ...
                                <TabsContent value="print">
                                    <Card>
                                        <CardHeader>
                                            ...
                                            <CardDescription>Use the print button at the top to print this report</CardDescription>
                                        </CardHeader>
                                        <CardContent>
                                            <div className="space-y-6">
                                                <div>
                                                    <h2 className="mb-2 text-lg font-bold">{t('report_summary')}</h2>
                                                    <p className="mb-4 text-muted-foreground">{getFilterSummary()}</p>
                                                    ...
                                                    <div className="mb-4 grid grid-cols-2 gap-4">
                                                        ...
                                                        <div className="rounded-md border p-4">
                                                            <div className="text-sm text-muted-foreground">{t('average_rental_value')}</div>
                                                            <div className="text-xl font-bold">{formatCurrency(averageAmount)}</div>
                                                        </div>
                                                        <div className="rounded-md border p-4">
                                                            <div className="text-sm text-muted-foreground">{t('average_duration')}</div>
                                                ...
                                                <div>
                                                    <h2 className="mb-4 text-lg font-bold">{t('rental_details')}</h2>
                                                    <Table>
                                                        <TableHeader>
                                                            <TableRow>
                                                                <TableHead>Rental #</TableHead>
                            ...
                            <div className="hidden print:block">
                                <div className="mb-8">
                                    <h1 className="mb-2 text-2xl font-bold">{t('rental_report')}</h1>
                                    <p className="mb-6 text-muted-foreground">{getFilterSummary()}</p>
                                ...
                                <div className="mb-8">
                                    <h2 className="mb-4 text-lg font-bold">{t('status_distribution')}</h2>
                                    <div className="mb-4 flex gap-4">
                                        {Object.entries(statusCounts).map(([status, count]) => (
                                            <div key={status} className="flex-1 rounded-md border p-4">
