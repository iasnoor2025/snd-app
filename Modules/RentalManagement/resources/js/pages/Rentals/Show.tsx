...
import { PermissionString, Rental } from '@/Core/types/models';
...
export default function Show({
    ...
    const getWeatherIcon = () => {
        ...
        return iconMap[weatherData.conditions.toLowerCase()] || <Cloud className="h-5 w-5 text-gray-500" />;
    };
                ...
                {address && (
                    <p className="text-sm text-muted-foreground">
                        ...
                        {(city || state || zipCode) && (
                            <>
                                <br />
                                <span className="ml-4">{[city, state, zipCode].filter(Boolean).join(', ')}</span>
                            </>
                        )}
                        {country && (
                            <>
    ...
    const handleDelete = () => {
        ...
        axios
            .delete(route('rentals.destroy', rental.id))
            .then(() => {
    ...
    const openSimpleExtensionDialog = () => {
        ...
        const newEndDateValue = rental.expected_end_date ? addDays(new Date(rental.expected_end_date), 7) : addDays(new Date(), 7);
    ...
    const handleSimpleExtensionSubmit = (e: React.FormEvent) => {
        ...
        axios
            .post(route('rentals.request-extension', rental.id), requestData)
            .then((response) => {
        ...
        <AppLayout>
            ...
            {rental.invoice_id && (
                <a
                    ...
                    className="mb-4 inline-block rounded bg-blue-600 px-4 py-2 text-white transition hover:bg-blue-700"
                >
            ...
            {!isLoading && !dataLoaded && (
                <div className="flex h-[50vh] w-full flex-col items-center justify-center gap-4">
                    ...
                    <Button onClick={() => (window.location.href = '/rentals')}>Return to Rentals List</Button>
                </div>
            )}
                    ...
                    <div className="grid gap-6">
                        ...
                        <Card>
                            <CardHeader className="flex flex-row items-center justify-between space-y-0">
                                <div>
                                    <CardTitle>
                                        {t('rental_number')}: {rental?.rental_number ?? '-'}
                                    </CardTitle>
                        ...
                        <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
                            ...
                            <div className="space-y-6 md:col-span-1">
                                ...
                                {/* Notifications Panel */}
                                <RentalNotificationsPanel notifications={notifications} />
                            </div>

                            {/* Right Column: Main Content */}
                            <div className="md:col-span-2">
                                {/* Workflow Status */}
                                <RentalWorkflowStatus rental={rental} nextPossibleStates={nextPossibleStates} className="mb-6" />
