...
export default function Index({ auth, rentals, filters = {} }: Props) {
    ...
    const columns = [
        ...
        },
        {
            ...
            cell: (row: Rental) => (
                <div className="flex items-center gap-2">
                    <Button variant="ghost" size="icon" onClick={(e) => handleViewClick(e, row.id)}>
                        <Eye className="h-4 w-4" />
                    </Button>
                    {canEditRentals && (
                        <Button variant="ghost" size="icon" onClick={() => router.get(route('rentals.edit', row.id))}>
                ...
                <Card>
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                        <div>
                            ...
                            <CardDescription>Manage your equipment rentals, track status, and process contracts.</CardDescription>
                        </div>
                        <div className="flex items-center space-x-2">
                            ...
                            <CreateButton resourceType="rentals" buttonText="Create Rental" href="/rentals/create" />
                        </div>
                    </CardHeader>
                    ...
                    <CardContent>
                        <div className="mb-6 space-y-4">
                            <form onSubmit={handleSearch} className="grid gap-4 md:grid-cols-4">
                                ...
                                <div>
                                    <Select value={data.status} onValueChange={handleStatusChange} name="status">
                                ...
                                <div>
                                    <div className="relative">
                                        <CalendarClock className="absolute top-2.5 left-2.5 h-4 w-4 text-muted-foreground" />
                                        <Input
                                            type="date"
                            ...
                            {(data.search || data.status !== 'all' || data.start_date || data.end_date) && (
                                <div className="mt-4 flex items-center justify-between border-t pt-2 pb-1">
                                    <div className="flex flex-wrap items-center gap-2">
                                        ...
                                        <Button variant="ghost" size="sm" onClick={handleReset} className="h-auto px-2 py-1 text-xs">
                                            Clear All
                                        </Button>
                                    </div>
                                </div>
                        ...
                        <div className="rounded-md border">
                            <DataTable columns={columns} data={rentals?.data || []} onRowClick={(row) => router.get(route('rentals.show', row.id))} />
                        </div>

                        {rentals?.last_page > 1 && (
                            <div className="mt-4 flex items-center justify-between">
                                <div className="text-sm text-muted-foreground">
                                    Showing {rentals?.current_page * rentals?.per_page - rentals?.per_page + 1} to{' '}
                                    {Math.min(rentals?.current_page * rentals?.per_page, rentals?.total)} of {rentals?.total} results
                                </div>
                ...
                <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
                    <DialogContent>
                        <DialogHeader>
                            ...
                            <DialogDescription>
                                This action cannot be undone. This will permanently delete the rental contract and remove its data from our servers.
                            </DialogDescription>
                        </DialogHeader>
                        <DialogFooter>
