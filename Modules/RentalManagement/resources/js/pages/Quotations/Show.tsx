...
import { ArrowLeft, Download, Home, Loader2, Mail, Printer } from 'lucide-react';
    ...
    return (
        <div className="container mx-auto py-8">
            ...
            <div className="mb-6 flex items-center gap-4">
                <a href={route('quotations.index')} className="flex items-center text-muted-foreground transition-colors hover:text-primary">
                    <ArrowLeft className="mr-1 h-4 w-4" />
                    {getTranslationString(t('back_to_quotations'), 'Back to Quotations')}
                </a>
                <nav className="ml-4 flex items-center gap-2 text-sm" aria-label="Breadcrumb">
                    <a href="/" className="flex items-center text-muted-foreground hover:text-primary">
                        <Home className="mr-1 h-4 w-4" />
                        {getTranslationString(t('home'), 'Home')}
                    </a>
                    <span className="mx-1">/</span>
                    <a href={route('quotations.index')} className="text-muted-foreground hover:text-primary">
                        {getTranslationString(t('quotations'), 'Quotations')}
                    </a>
                    <span className="mx-1">/</span>
                    <span className="font-semibold text-primary">
                        {getTranslationString(t('quotation_number'), 'Quotation Number')}: {quotation.quotation_number}
                    </span>
                </nav>
                <div className="ml-auto flex gap-2">
                    <Button
                        ...
                        size="sm"
                        onClick={() => {
                            toast.info(getTranslationString(t('printing'), 'Printing'));
                            window.open(`/quotations/${quotation.id}/print`, '_blank');
                        }}
                    >
                        <Printer className="mr-1 h-4 w-4" /> {getTranslationString(t('print'), 'Print')}
                    </Button>
                    <Button
                        variant="outline"
                        size="sm"
                        onClick={() => {
                            toast.info(getTranslationString(t('downloading_pdf'), 'Downloading PDF'));
                            window.open(`/quotations/${quotation.id}/pdf`, '_blank');
                        }}
                    >
                        <Download className="mr-1 h-4 w-4" /> {getTranslationString(t('download_pdf'), 'Download PDF')}
                    </Button>
                    <Button variant="outline" size="sm" onClick={handleSendEmail} disabled={emailLoading}>
                        {emailLoading ? <Loader2 className="mr-1 h-4 w-4 animate-spin" /> : <Mail className="mr-1 h-4 w-4" />}
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>
                        {getTranslationString(t('customer'), 'Customer')}: {safeString(quotation.customer?.company_name)}
                    </CardDescription>
                </CardHeader>
                <CardContent>
                    ...
                    <div className="mb-6 grid grid-cols-1 gap-6 md:grid-cols-2">
                        ...
                        <Card className="bg-muted/30">
                            ...
                            <CardContent>
                                <div className="grid gap-1 text-sm">
                                    <div>
                                        <b>{getTranslationString(t('rental_number'), 'Rental Number')}:</b>{' '}
                                        {safeString(quotation.rental?.rental_number)}
                                    </div>
                                    <div>
                                        <b>{getTranslationString(t('rental_status'), 'Rental Status')}:</b> {safeString(quotation.rental?.status)}
                                    </div>
                                    <div>
                                        <b>{getTranslationString(t('rental_period'), 'Rental Period')}:</b> {safeString(quotation.rental?.start_date)}{' '}
                                        — {safeString(quotation.rental?.expected_end_date)}
                                    </div>
                                </div>
                            </CardContent>
                        </Card>
                        ...
                        <Card className="bg-muted/30">
                            ...
                            <CardContent>
                                <div className="grid gap-1 text-sm">
                                    <div>
                                        <b>{getTranslationString(t('contact_person'), 'Contact Person')}:</b>{' '}
                                        {safeString(quotation.customer?.contact_person)}
                                    </div>
                                    <div>
                                        <b>{getTranslationString(t('email'), 'Email')}:</b> {safeString(quotation.customer?.email)}
                                    </div>
                                    <div>
                                        <b>{getTranslationString(t('phone'), 'Phone')}:</b> {safeString(quotation.customer?.phone)}
                                    </div>
                                </div>
                            </CardContent>
                        </Card>
                    ...
                    <div className="mb-4 grid grid-cols-2 gap-2">
                        <div>
                            <b>{getTranslationString(t('status'), 'Status')}:</b> {getTranslationString(quotation.status, quotation.status)}
                        </div>
                        <div>
                            <b>{getTranslationString(t('issue_date'), 'Issue Date')}:</b>{' '}
                            {getTranslationString(quotation.issue_date, quotation.issue_date)}
                        </div>
                        <div>
                            <b>{getTranslationString(t('valid_until'), 'Valid Until')}:</b>{' '}
                            {getTranslationString(quotation.valid_until, quotation.valid_until)}
                        </div>
                        <div>
                            <b>{getTranslationString(t('total_amount'), 'Total Amount')}:</b>{' '}
                            {getTranslationString(quotation.total_amount, quotation.total_amount)}
                        </div>
                        <div>
                            <b>{getTranslationString(t('notes'), 'Notes')}:</b> {safeString(quotation.notes)}
                        </div>
                    </div>
                    <div className="overflow-x-auto">
                        <h3 className="mb-2 font-semibold">{getTranslationString(t('quotation_items'), 'Quotation Items')}</h3>
                                ...
                                {quotationItems.data.length === 0 ? (
                                    <tr>
                                        <td colSpan={5} className="py-4 text-center">
                                            {getTranslationString(t('no_items'), 'No items')}
                                        </td>
                                    </tr>
                                ) : (
                    ...
                    <div className="mt-4 flex flex-wrap gap-2">
                        {canApprove && (
                            <Button onClick={handleApprove} disabled={actionLoading === 'approve'}>
                                ...
                                {getTranslationString(t('approve'), 'Approve')}
                            </Button>
                        )}
                        {canReject && (
                            <Button variant="destructive" onClick={handleReject} disabled={actionLoading === 'reject'}>
                                ...
                                {getTranslationString(t('reject'), 'Reject')}
                            </Button>
                        )}
                        {canEdit && (
                            <Button variant="outline" asChild>
                                <a href={route('quotations.edit', quotation.id)}>{getTranslationString(t('edit'), 'Edit')}</a>
                            </Button>
                        )}
                        {canDelete && (
                            <>
                                ...
                                <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
                                    <AlertDialogContent>
                                        <AlertDialogHeader>
                                            ...
                                            <AlertDialogDescription>
                                                {getTranslationString(t('delete_quotation_warning'), 'Are you sure?')}
                                            </AlertDialogDescription>
                                        </AlertDialogHeader>
                                        <AlertDialogFooter>
