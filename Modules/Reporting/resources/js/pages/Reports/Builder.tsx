...
import { Card, CardContent, CardHeader, CardTitle, Button, Input, Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/Core';
    ...
    <AppLayout>
      ...
      <div className="container mx-auto py-6">
        ...
        <div className="mb-8">
          ...
          <p className="text-muted-foreground mb-4">Build custom reports from any data source.</p>
        </div>
        <Card className="mb-8">
          <CardHeader>
          ...
          <CardContent>
              ...
              <Select value={dataSource} onValueChange={setDataSource}>
                ...
                <SelectContent>
                  {props.dataSources.map((src) => (
                    <SelectItem key={src} value={src}>{src.split('\\').pop()}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
              ...
              <div className="flex flex-wrap gap-2">
                {availableColumns.map((col) => (
                  <Button
                    ...
                    size="sm"
                    onClick={() => setSelectedColumns(selectedColumns.includes(col)
            ...
            <div className="mb-4">
              ...
              <Select value={aggregation} onValueChange={setAggregation}>
                <SelectTrigger><SelectValue placeholder="Select aggregation" /></SelectTrigger>
                <SelectContent>
              ...
              {filters.map((filter, idx) => (
                <div key={idx} className="flex gap-2 mb-2 items-center">
                  <Select value={filter.column} onValueChange={v => updateFilter(idx, 'column', v)}>
                    <SelectTrigger className="w-32"><SelectValue placeholder="Column" /></SelectTrigger>
                    <SelectContent>
                      {availableColumns.map((col) => (
                  ...
                  <Input
                    ...
                    placeholder="Value"
                  />
                  <Button variant="destructive" size="icon" onClick={() => removeFilter(idx)}>-</Button>
                </div>
              ))}
            ...
            {/* Visualization Type */}
            <div className="mb-4">
              <label className="block mb-1 font-medium">Visualization</label>
              <Select value={visualization} onValueChange={setVisualization}>
                <SelectTrigger><SelectValue placeholder="Select visualization" /></SelectTrigger>
                <SelectContent>
            ...
            {/* Export Format */}
            <div className="mb-4">
              <label className="block mb-1 font-medium">Export Format</label>
            ...
            <div className="mb-4">
              ...
              {schedule.frequency === 'weekly' && (
                <Select value={schedule.day} onValueChange={v => setSchedule(s => ({ ...s, day: v }))}>
                  <SelectTrigger className="mt-2"><SelectValue placeholder="Day of Week" /></SelectTrigger>
                  <SelectContent>
              ...
              {schedule.frequency === 'monthly' && (
                <Select value={schedule.day} onValueChange={v => setSchedule(s => ({ ...s, day: v }))}>
                  <SelectTrigger className="mt-2"><SelectValue placeholder="Day of Month" /></SelectTrigger>
                  <SelectContent>
                    {props.daysOfMonth.map((d) => (
                      <SelectItem key={d.toString()} value={d.toString()}>{d}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              )}
              <Select value={schedule.time} onValueChange={v => setSchedule(s => ({ ...s, time: v }))}>
                <SelectTrigger className="mt-2"><SelectValue placeholder="Time" /></SelectTrigger>
                <SelectContent>
                  {props.times.map((t) => (
        ...
        {error && (
          <Card className="mt-8 border-red-500">
            ...
            <CardContent><div className="text-red-600">{error}</div></CardContent>
          </Card>
        )}
        {result && (
          <Card className="mt-8">
            <CardHeader><CardTitle>Report Result</CardTitle></CardHeader>
            <CardContent>
              {result.data && Array.isArray(result.data) && result.data.length > 0 ? (
                ...
                <pre className="text-xs bg-muted p-2 rounded overflow-x-auto">{JSON.stringify(result, null, 2)}</pre>
              )}
            </CardContent>
          </Card>
        )}
