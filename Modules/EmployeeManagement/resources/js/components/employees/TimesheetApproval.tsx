...
import { AlertCircle, Check, Clock, Eye, Filter, User, X } from 'lucide-react';
...
export const TimesheetApproval: React.FC<TimesheetApprovalProps> = ({ initialEntries = [] }) => {
    ...
    const approveEntry = async (entryId: number) => {
        try {
            ...
            setEntries((prev) => prev.map((entry) => (entry.id === entryId ? { ...entry, status: 'approved' } : entry)));
    ...
    const rejectEntry = async () => {
        ...
        try {
            ...
            // Update the entry in the list
            setEntries((prev) =>
                prev.map((entry) => (entry.id === currentEntry.id ? { ...entry, status: 'rejected', notes: rejectionNote } : entry)),
            );
    ...
    const executeBulkAction = async () => {
        ...
        try {
            if (bulkAction === 'approve') {
                ...
                setEntries((prev) => prev.map((entry) => (selectedEntries.includes(entry.id) ? { ...entry, status: 'approved' } : entry)));
            } else {
                ...
                // Update entries in the list
                setEntries((prev) =>
                    prev.map((entry) => (selectedEntries.includes(entry.id) ? { ...entry, status: 'rejected', notes: rejectionNote } : entry)),
                );
            }
    ...
    return (
        <div className="space-y-6">
                ...
                <CardHeader className="pb-3">
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <p className="mt-1 text-sm text-gray-500">{pendingEntriesCount} pending timesheet entries require approval</p>
                        </div>

                        {selectedEntries.length > 0 && (
                            <div className="flex gap-2">
                                <Button variant="outline" onClick={() => openBulkActionDialog('approve')} className="flex items-center gap-1">
                                    {<Check className="h-4 w-4" />}
                                    Approve Selected ({selectedEntries.length})
                                </Button>
                                <Button variant="outline" onClick={() => openBulkActionDialog('reject')} className="flex items-center gap-1">
                                    {<X className="h-4 w-4" />}
                        ...
                        <div className="mb-6 rounded-md border p-4">
                            ...
                            <div className="grid grid-cols-1 gap-4 md:grid-cols-5">
                                ...
                                <div>
                                    ...
                                    <Select value={filter.employee_id} onValueChange={(value) => handleFilterChange('employee_id', value)}>
                                        <SelectTrigger>
                            ...
                            <Table>
                                ...
                                <TableBody>
                                    {loading ? (
                                        ...
                                        <TableRow>
                                            <TableCell colSpan={7} className="py-10 text-center">
                                                <p className="mb-2 text-gray-500">No timesheet entries found.</p>
                            ...
                            <div className="rounded-md bg-gray-50 px-3 py-1.5">
                                <div className="flex items-center gap-3">
                                    <div>
                                        ...
                                        {entries.reduce((acc, entry) => acc + entry.hours_worked + entry.overtime_hours, 0).toFixed(2)} hrs
                                    </div>
                                    <div>
            ...
            <Dialog open={detailDialogOpen} onOpenChange={setDetailDialogOpen}>
                <DialogContent className="max-w-2xl">
                    <DialogHeader>
                        ...
                        <DialogDescription>Review the details of this timesheet entry.</DialogDescription>
                    </DialogHeader>
                        ...
                        <div className="space-y-6 py-4">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="rounded-md bg-gray-50 p-3">
                                    <h4 className="mb-2 text-sm font-medium">Date & Time</h4>
                                ...
                                <div className="rounded-md bg-gray-50 p-3">
                                    <h4 className="mb-2 text-sm font-medium">Hours</h4>
                            ...
                            {currentEntry.status === 'pending' && (
                                <div className="flex justify-end gap-2 pt-4">
                                    <Button variant="outline" onClick={() => openRejectDialog(currentEntry)} className="flex items-center gap-1">
                                        {<X className="h-4 w-4" />}
                                        Reject
                                    </Button>
                                    <Button onClick={() => approveEntry(currentEntry.id)} className="flex items-center gap-1">
                                        {<Check className="h-4 w-4" />}
                                        Approve
                                    </Button>
                                </div>
            ...
            <AlertDialog open={rejectDialogOpen} onOpenChange={setRejectDialogOpen}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            Please provide a reason for rejecting this timesheet entry. The employee will be notified of the rejection and the reason.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    ...
                    <AlertDialogFooter>
                        ...
                        <AlertDialogAction onClick={rejectEntry} className="bg-red-500 hover:bg-red-600" disabled={!rejectionNote.trim()}>
                            Reject Entry
                        </AlertDialogAction>
                    </AlertDialogFooter>
                </AlertDialogContent>
            ...
            <AlertDialog open={bulkActionDialogOpen} onOpenChange={setBulkActionDialogOpen}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        <AlertDialogTitle>{bulkAction === 'approve' ? 'Approve Selected Entries' : 'Reject Selected Entries'}</AlertDialogTitle>
                        <AlertDialogDescription>
