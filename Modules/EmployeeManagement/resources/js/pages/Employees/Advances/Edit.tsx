...
export default function Edit({ auth, employee, advance }: Props) {
    ...
    const [paymentDate, setPaymentDate] = useState(advance.payment_date ? format(new Date(advance.payment_date), 'yyyy-MM-dd') : '');
    const [monthlyDeduction, setMonthlyDeduction] = useState(advance.monthly_deduction?.toString() || '');
                        ...
                        <form onSubmit={handleSubmit} className="space-y-6">
                            <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                                ...
                                <div className="space-y-2">
                                    ...
                                    />
                                    <p className="text-sm text-muted-foreground">The monthly amount to deduct from the employee's salary</p>
                                </div>
                            ...
                            <div className="flex justify-end gap-4">
                                <Button type="button" variant="outline" onClick={() => router.visit(`/employees/${employee.id}/advances`)}>
                                    Cancel
                                </Button>
