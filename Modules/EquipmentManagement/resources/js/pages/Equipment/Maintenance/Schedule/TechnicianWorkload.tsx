...
export function TechnicianWorkload({ workload }: TechnicianWorkloadProps) {
    ...
    return (
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-3">
            {workload.length === 0 ? (
                <div className="col-span-full py-8 text-center text-muted-foreground">No technicians assigned to maintenance tasks</div>
            ) : (
                workload.map((data) => (
                    <Card key={data.technician.id} className="overflow-hidden">
                        <div className="flex items-center space-x-4 bg-muted p-4">
                            <Avatar className="h-12 w-12">
                                <AvatarImage src={data.technician.profile_photo_url} alt={data.technician.name} />
                                ...
                                <div className="space-y-3">
                                    ...
                                    <ul className="max-h-40 space-y-2 overflow-y-auto pr-2">
                                        {data.schedule.map((record) => (
                                            <li key={record.id} className="border-l-2 border-blue-500 py-1 pl-3 text-sm">
