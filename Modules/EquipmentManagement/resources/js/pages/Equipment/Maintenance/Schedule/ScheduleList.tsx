...
import { AlertTriangle, Calendar, MoreHorizontal, User, WrenchIcon } from 'lucide-react';
                    ...
                    {schedule.length === 0 ? (
                        <TableRow>
                            <TableCell colSpan={6} className="py-6 text-center text-muted-foreground">
                                {isConflict ? 'No scheduling conflicts found' : 'No maintenance records scheduled for this period'}
                            </TableCell>
                        </TableRow>
                    ) : (
                        schedule.map((record) => (
                            <TableRow key={record.id} className={isConflict ? 'bg-red-50' : ''}>
                                ...
                                <TableCell>{format(parseISO(record.scheduled_date), 'MMM d, yyyy')}</TableCell>
                                <TableCell>
                                    <div className="flex items-center">
                                        {isConflict && <AlertTriangle className="mr-2 h-4 w-4 text-red-500" />}
