...
import { Slider } from '../../../../../../../resources/js/components/ui/slider';
...
const DigitalTwinVisualization: React.FC<Props> = ({
  equipment,
  digitalTwinData,
  scenarios = []
}) => {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // Main equipment body
      ctx.fillStyle = equipment.status === 'running' ? '#10b981' :
      ...
      components.forEach((component, index) => {
        ...
        const healthColor = component.health > 80 ? '#10b981' :
                           component.health > 60 ? '#f59e0b' : '#ef4444';
  ...
  const toggleComponentVisibility = (componentId: string) => {
    setComponents(prev => prev.map(comp =>
      comp.id === componentId ? { ...comp, isVisible: !comp.isVisible } : comp
    ));
  };
  ...
  const getComponentTypeIcon = (type: string) => {
    switch (type) {
      ...
      case 'hydraulic': return <Gauge className="h-4 w-4" />;
          ...
          <div className={isFullscreen ? 'col-span-1' : 'lg:col-span-3'}>
            <Card className="h-[600px]">
              ...
              <CardContent className="h-full p-0">
                <canvas
                  ...
                  onMouseDown={(e) => {
                    ...
                    components.forEach((comp, index) => {
                      ...
                      if (x >= compX - size/2 && x <= compX + size/2 &&
                          y >= compY - size/2 && y <= compY + size/2) {
                ...
                {/* 3D Controls Overlay */}
                <div className="absolute top-4 right-4 bg-black/80 rounded-lg p-3 space-y-2">
                  <div className="flex items-center gap-2">
                    <Label className="text-white text-xs">Auto Rotate</Label>
                    <Switch checked={autoRotate} onCheckedChange={setAutoRotate} />
                  </div>
                  <div className="flex items-center gap-2">
                    <Label className="text-white text-xs">Show Metrics</Label>
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      ...
                      <div>
                        ...
                        <div className="font-medium">
                          {selectedComponent.health > 80 ? 'Good' :
                           selectedComponent.health > 60 ? 'Fair' : 'Poor'}
                        </div>
                      </div>
                    </div>
                ...
                {selectedScenario && (
                  <>
                    <div className="p-3 bg-gray-50 rounded-lg">
                      <h4 className="font-medium text-sm mb-2">{selectedScenario.name}</h4>
                      <p className="text-xs text-muted-foreground mb-3">{selectedScenario.description}</p>
                      <div className="grid grid-cols-2 gap-2 text-xs">
